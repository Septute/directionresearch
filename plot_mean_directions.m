function plot_mean_directions(start_date, end_date)
    % Load the data
    ...
    % Convert input dates to datenum
    start_date_num = datenum(start_date);
    end_date_num = datenum(end_date);

    % Select data for the specified dates
    selected_dates_idx = date_nums >= start_date_num & date_nums <= end_date_num;
    ...
    % Circular plot
    figure;
    compass(deg2rad(mean_wind_direction), mean_magnitude,'m-*');
    hold on;
    compass(deg2rad(mean_wave_direction), mean_magnitude, 'b-s');
    compass(deg2rad(mean_current_direction), mean_magnitude, 'g-^');
    compass(deg2rad(mean_stress_direction), mean_magnitude, 'k');
    legend('Wind Direction', 'Wave Direction', 'Current Direction', 'Stress Direction');
    title(['Mean Directions (', datestr(start_date), ' to ', datestr(end_date), ')']);
    hold off;
end