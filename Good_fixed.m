...
% Identify and remove outliers in Uz
Q1 = quantile(data.Uz(valid_idx), 0.25);
Q3 = quantile(data.Uz(valid_idx), 0.75);
IQR = Q3 - Q1;
lower_bound = Q1 - 1.5 * IQR;
upper_bound = Q3 + 1.5 * IQR;

valid_idx = valid_idx & data.Uz >= lower_bound & data.Uz <= upper_bound; % Remove outliers
...
mvalid_idx = r2_Tw_p >= .9 & r2_uw_p >= .9 & valid_idx & Uz_p >= lower_bound & Uz_p <= upper_bound; 
...
scatter(shifted_Wdir_measured_p, adjusted_wave_direction, 20, 'o', 'MarkerFaceColor', 'm', 'DisplayName', 'Wave Direction');
scatter(shifted_Wdir_measured_p, adjusted_current_direction, 20, 'o', 'MarkerFaceColor', 'b', 'DisplayName', 'Current Direction');
...
for i = 1:length(bin_centers)
    ...
    % Wave Direction
    bin_data_wave = adjusted_wave_direction(bin_idx);
    ...
    % Current Direction
    bin_data_current = adjusted_current_direction(bin_idx);
