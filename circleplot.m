...
% Polar plot for wind direction and magnitude
figure;
polarplot(deg2rad(filtered_Wdir_measured), data.Uz(valid_idx), 'o-');
thetaticks(0:45:360);
rticks(round(max(data.Uz(valid_idx)) * (0.2:0.2:1), 1)); 
title('Wind Direction and Magnitude Over Time');

% Polar plot for wave direction and magnitude
figure;
polarplot(deg2rad(adjusted_wave_direction), adjusted_wave_height, 'o-');
thetaticks(0:45:360);
rticks(round(max(adjusted_wave_height) * (0.2:0.2:1), 1)); 
title('Wave Direction and Magnitude Over Time');

% Polar plot for current direction and magnitude
figure;
polarplot(deg2rad(adjusted_current_direction), adjusted_current_speed, 'o-');
thetaticks(0:45:360);
rticks(round(max(adjusted_current_speed) * (0.2:0.2:1), 1)); 
title('Current Direction and Magnitude Over Time');

% Filtered Ustar data
filtered_Ustar = data.Ustar(valid_idx);

% Polar plot for stress direction with U* as magnitude
figure;
polarplot(deg2rad(filtered_Sdir_measured), filtered_Ustar, 'o-');
thetaticks(0:45:360);
rticks(round(max(filtered_Ustar) * (0.2:0.2:1), 1)); 
title('Stress Direction and U* Magnitude Over Time');